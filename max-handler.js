...
async function handleMAXWebhook(req, res) {
    ...
    try {
        ...
        if (update.update_type === 'bot_started') {
            const chatId = update.chat_id;           // chat_id для отправки
            const userId = update.user.user_id;      // user_id для привязки
        ...
        if (update.update_type === 'message_created') {
            ...
            const chatId = message.recipient.chat_id; // chat_id для отправки
            const userId = message.sender.user_id;     // user_id для привязки
